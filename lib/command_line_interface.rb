...
def current_user
  if is_a_returning_user.class == User
    return is_a_returning_user
  elsif create_account.class == User
...
def blood_pressure_result(blood_pressure_input)
  ...
  elsif (x[0] <= 180 && x[1] >= 90) && (x[0] >= 140 && x[1] <=120)
    puts "high"
  elsif (x[0] > 180 && x[1] > 120)
    puts "Your blood pressure is dangerously high, please see a doctor ASAP!"
  elsif (x[0] <= 90 && x[1] <= 60) && (x[0] > 50 && x[1] > 33)
    puts "low"
  else
...
def give_user_low_sodium_food_recs
  food_rec = FoodRec.create(user_id: 2, food_rec1: low_sodium_rec[0].name ,food_rec2: low_sodium_rec[1].name, food_rec3: low_sodium_rec[2].name, food_rec4: low_sodium_rec[3].name,food_rec5: low_sodium_rec[4].name)
  food_rec
end

 def give_user_high_sodium_food_recs
  FoodRec.create(user_id: current_user.id, food_rec1: high_sodium_rec[0].name ,food_rec2: high_sodium_rec[1].name, food_rec3: high_sodium_rec[2].name, food_rec4: high_sodium_rec[3].name,food_rec5: high_sodium_rec[4].name)
 end
